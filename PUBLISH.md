...
This document describes how to publish `cylera-cli` to PyPI using the automated release workflow.
...
- Run all CI checks (shellcheck, bandit, pip-audit, unit tests, integration tests)
- Publish to TestPyPI
- Verify the package installs and runs correctly from TestPyPI
...
- Run all CI checks
- Publish to PyPI
- Verify the package installs and runs correctly from PyPI
...
| `CYLERA_BASE_URL` | Repository secrets | Integration test API endpoint |
| `CYLERA_USERNAME` | Repository secrets | Integration test credentials |
| `CYLERA_PASSWORD` | Repository secrets | Integration test credentials |
